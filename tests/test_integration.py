...
async def test_password_reset_flow(client: AsyncClient) -> None:
    ...
    r = await client.post("/api/v1/auth/password/reset/confirm", json={"token": reset_token, "new_password": new_password})
    assert r.status_code == 200, r.text
    ...
    r = await client.post("/api/v1/auth/password/reset/confirm", json={"token": reset_token, "new_password": "another123"})
...
async def test_audit_logs_on_m2m_token(client: AsyncClient) -> None:
    ...
    async with async_session_factory() as session:
        audit = (await session.exec(
            select(AuditLog).where(AuditLog.event_type == "m2m_token_generated")
        )).first()
        assert audit is not None
        assert audit.meta.get("success") is True
...
async def test_audit_logs_on_webhook_created(client: AsyncClient) -> None:
    ...
    r = await client.post("/api/v1/webhooks/endpoints", json={
    ...
    async with async_session_factory() as session:
        audit = (await session.exec(
            select(AuditLog).where(AuditLog.event_type == "webhook_created")
        )).first()
        assert audit is not None
